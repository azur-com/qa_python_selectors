...
def test_dark_theme_by_time():
    ...
    is_dark_theme = False
    if  current_time >= time(22) or (current_time <= time(6)): is_dark_theme = True
    print(current_time)
...
def test_find_suitable_user():
    ...
    suitable_users = [x for x in users if x["name"]=="Olga"][0]
    print(suitable_users)
    assert suitable_users == {"name": "Olga", "age": 45}

    # TODO найдите всех пользователей младше 20 лет
    suitable_users = [x for x in users if x["age"]<20]
